...
function frame(){
  ...
  /* The camera used to move a fixed fraction of the gap per frame. On a slow
     or throttled frame rate that let the runner pull away for ever, so you
     watched an empty board while the score climbed. Chase on the clock
     instead, and if somehow left far behind, just cut to them. */
  var now = performance.now();
  var dt = Math.min(0.1, (now - (frame.last || now)) / 1000);
  frame.last = now;

  /* A low frame rate stretches the chase out, so cut rather than crawl once
     the runner is further away than the screen can comfortably hold. */
  if (Math.abs(r.x - cam.x) > 7 || Math.abs(r.y - cam.y) > 6){
    cam.x = r.x; cam.y = r.y;
  } else {
    var kx = 1 - Math.pow(0.0001, dt);     /* about a tenth of a second */
    var ky = 1 - Math.pow(0.02, dt);       /* gentler vertically */
    cam.x += (r.x - cam.x) * kx;
    cam.y += (r.y - cam.y) * ky;
  }
